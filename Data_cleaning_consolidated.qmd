...
formatted_Toxicity <- Toxicity %>%
  mutate(
    ...
    # Create binary indicators (0 and 1 instead of NA and 1)
    `Acute Toxic` = if_else(str_detect(all_toxicity, "Acute"), 1, 0),
    Corrosive = if_else(str_detect(all_toxicity, "Corrosive"), 1, 0),
    `Environmental Hazard` = if_else(str_detect(all_toxicity, "Environmental"), 1, 0),
    Flammable = if_else(str_detect(all_toxicity, "Flammable"), 1, 0),
    `Health Hazard` = if_else(str_detect(all_toxicity, "Health"), 1, 0),
    Irritant = if_else(str_detect(all_toxicity, "Irritant"), 1, 0)
  ) %>%
...
write.csv(strawberry_with_toxicity,'strawberry_with_toxicity.csv', row.names = FALSE)
```
