...
unique(Strawberry_clean$Domain.Category)
unique(Strawberry_clean$Data.Item)
...
head(Strawberry_clean)
# Implement drop column Function
Strawberry_clean <- drop_one_value_col(Strawberry_clean)
```

```{r}
unique(Strawberry_clean$Domain)
```
